...
- [PG-004] **锁定产出**：决策树解析完且与用户达成共识后，必须调用文件写入工具，在当前工作区根创建 `.plan-reviews/<plan-slug>/PLAN.md`（Goal / Constraints & assumptions / Approach / Key decisions & tradeoffs / Validation plan / Risks / Out of scope），随后读回验证文件存在且内容完整。写入或验证失败时必须报告阻塞，禁止声称“已锁定”。**确认前不执行计划。**
