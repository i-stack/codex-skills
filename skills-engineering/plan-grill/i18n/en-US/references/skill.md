...
- [PG-004] **Lock output**: After the decision tree is resolved and consensus is reached, invoke [scripts/write_plan.py](../../../scripts/write_plan.py) to create and verify `.plan-reviews/<plan-slug>/PLAN.md` under the current workspace root (Goal / Constraints & assumptions / Approach / Key decisions & tradeoffs / Validation plan / Risks / Out of scope). If the script exits nonzero, report the block and never claim the plan is locked. **Do not execute the plan before confirmation.**
