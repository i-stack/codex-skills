...
- 盘问规则**复用 `plan-grill`**（PG-000~PG-006），历史召回**复用 `historical-recall`**（HR-001~HR-005），本 skill 不内联重复这两套规则，只做「澄清 + 改写 + 展示 + 回答 + 回写」这层增量。
...
- [POPT-003] 盘问判定与澄清：复用 plan-grill PG-000 门控，存在阻塞性歧义时先逐一盘问澄清，无歧义直接改写。
...
| `plan-grill` | 需求盘问与计划锁定（POPT-003 委托其 PG-000 门控 + PG-001~006） |
